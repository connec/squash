(function() {
  ...
  exports.Squash = (function() {
    ...
    Squash.prototype.squash = function() {
      var ast, directory, file, id, module, names, obfuscate, obfuscated, output, path, _i, _j, _len, _len2, _ref, _ref2, _ref3;
      ...
      obfuscate = function(names) {
        var from, result, _ref2;
        result = {};
        for (from in names) {
          result[(_ref2 = obfuscated[from]) != null ? _ref2 : obfuscated[from] = id++] = names[from];
        }
        return result;
      };
      _ref2 = this.ordered;
      for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
        ...
        if (this.options.obfuscate) {
          directory = ((_ref3 = obfuscated[directory]) != null ? _ref3 : obfuscated[directory] = id++);
          names = obfuscate(names);
        }
        output += ";register(" + (util.inspect(names)) + ", " + (util.inspect(directory)) + ", function(module, exports, require) {;\n  " + module.js + "\n;});";
      }
      output += '\n;}).call(this);';
    ...
    Squash.prototype.require = function(name, from) {
      ...
      if (this.modules[file] != null) {
        names = ((_ref = (_base = this.modules[file].names)[_name = path.relative(this.cwd, from)]) != null ? _ref : _base[_name] = []);
        if (__indexOf.call(names, name) < 0) names.push(name);
        return;
      }
      this.modules[file] = {
        directory: path.relative(this.cwd, path.dirname(file)),
        js: fs.readFileSync(file, 'utf8'),
        names: {}
      };
      this.modules[file].names[path.relative(this.cwd, from)] = [name];
